...
node('php') {
    ...
    stage("Tests - unit") {
        ...
        try {
            sh 'vendor/bin/coverage-check /tmp/coverage_unit.xml 75'
        } catch (e) {
            echo 'Unit test coverage is below 75%. Please add coverage.'
            throw e
        }
    }
    ...
    stage("Tests - integration") {
        ...
        try {
            sh 'vendor/bin/coverage-check /tmp/coverage_integration.xml 84'
        } catch (e) {
            echo 'Integration test coverage is below 84%. Please add coverage.'
            throw e
        }
    }
}